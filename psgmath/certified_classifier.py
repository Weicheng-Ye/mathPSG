...
def _backend_external_artifact_snapshot(
    ...
    if store_before is None:
        ...
        from .task5_release import (
            Task5ReleaseStore,
            _verify_task5_release_store_issuance,
        )

        if type(store_before) is not Task5ReleaseStore:
            raise TypeError(
                "classifier backend Task5 release store must be an exact capability"
            )
        _verify_task5_release_store_issuance(store_before)
        expected = ("task5-release-store", _raw_digest(store_before.manifest_bytes))
        if task5_rows != (expected,):
            raise MalformedCertificateError(
                "classifier backend Task5 binding differs from its exact issued store"
            )
...
class ClassifierBackendAuthority:
    ...
    def external_artifact_bindings(self) -> tuple[tuple[str, str], ...]:
        ...
        if type(store) is not Task5ReleaseStore:
            raise TypeError(
                "classifier backend Task5 release store must be an exact capability"
            )
        _verify_task5_release_store_issuance(store)
        return (("task5-release-store", _raw_digest(store.manifest_bytes)),)
